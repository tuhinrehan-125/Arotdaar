...
class CustomerController extends Controller
{
    ...
    {
        $this->middleware('auth:sanctum', ['except' => ['index','customerSearch']]);
    }
    ...
    public function index()
    {
        ...
        $customer = Customer::where('user_id',$userid)->where('delete_status', 1)->get();
    ...
    public function customerSearch(Request $request)
    {
        $name=$request->name;
        if($name){
        $customers = Customer::where('user_id',auth()->user()->id)->where('name', 'LIKE', "%$name%")->get();
