@extends('layouts.main', ['activePage' => 'collection', 'titlePage' => __('Collection')])
        ...
        <div class="container-fluid">
            ...
            <div class="card mb-3">
                <div class="card-header bg-primary text-white">
                    ...
                    Advance
                    <a href="{{route('collection.create')}}" class="text-white offset-10" style="height:20px;">
                        <span style="height:20px; font-size: 150%;" class="fa fa-plus">Add Collection</span>
                    </a>
                </div>
                                        ...
                                        <table class="table table-striped category-table" id="dataTable">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{$key->customer->mobile_no ?? ''}}</td>
                                                    <td>{{$key->amount ?? ''}}</td>
                                                    <td>{{$key->payment_method ?? ''}}</td>
                                                    <td>
                                                        <table>
                                                            <tr>
                                                                <td><button class="btn btn-sm btn-primary edit" data-id="{{$key->id ?? ''}}" data-customer_id="{{$key->customer->id ?? ''}}" data-payment_method="{{$key->payment_method ?? ''}}" data-amount="{{$key->amount ?? ''}}"><i class="fa fa-edit"></i>Edit</button></td>
                                                                <td><a class="btn btn-sm btn-danger delete" onclick="return confirm('do you want to delete')" href="{{route('collection.delete',$key->id)}}"><i class="fa fa-trash"></i>Delete</a></td>
            ...
            <div class="modal-body">
                ...
                <form id="collection-form" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select name="customerId" id="customerId" class="customerId form-control ">
...
<script>
    $(document).ready(function() {
        ...
        $('.edit').click(function(e) {
            ...
            $("#collection-id").val($(this).data('id'))
            $("#name").val($(this).data('collection_name'))
            $("#paymentMethod").val($(this).data('payment_method'))
            $("#paymentAmount").val($(this).data('amount'))

            var customer = $(this).data('customer_id')

            $('#customerId').val(customer).prop('selected', true);

            $('.modal-title').html('Update Collection')
            $("#collection-form").attr('action', "{{route('collection.update')}}")
